...
class Login extends CI_Controller {
        ...
        else
        {
            ...
            if($option=='individual')
            {
                ...
                if(count($usr_count) > 0)
                {
                    ...
                    $message='<html><body>Please click on the link to reset your Password <a href='.base_url().'/Login/forgotpwd/'.base64_encode($EmailAddress).'>Click Here</a></p></body></html>';
                    ...
                    // $this->session->set_flashdata("success","Email sent successfully."); 
                        echo "<div class='alert alert-success text-center mb-4' role='alert' id='message_div1'> Email sent successfully.<span class='remove1'>X</span></div>";
                    }
                    else
                    { 
                    // $this->session->set_flashdata("success","Error in sending Email.");
                        echo "<div class='alert alert-danger text-center mb-4' role='alert' id='message_div1'> Error in sending Email.<span class='remove1'>X</span></div>"; 
                    }
                }
                else
                {
                    // $this->session->set_flashdata('error','Given email address is not available in our database or Invalid email address');
                    echo "<div class='alert alert-danger text-center mb-4' role='alert' id='message_div1'> Given email address is not available or Invalid email address.<span class='remove1'>X</span></div>";
                }
            }
            else
            {
    ...
    public function resetpwd()
    {
        ...
        else
        {
            ...
            redirect('login/forgotpwd/'.base64_encode($mailid));
        }
    }
}