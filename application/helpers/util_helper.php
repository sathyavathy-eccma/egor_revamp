...
if (!function_exists('get_countries')) {
    ...
    function get_countries() {
        ...
        // $sql = "SELECT distinct member_type FROM `individual` order by member_type ASC";
...
if (!function_exists('get_cities')) {
    ...
    function get_cities($state) {
        $ci = & get_instance();
        $qry = $ci->db->query("SELECT * FROM states WHERE state_name='$state'");
...
if (!function_exists('get_form_fields')) {
    ...
    {
        $ci = & get_instance();
       // $ci->eccma_forms = $ci->load->database('eccma_forms', TRUE);

        $qry = $ci->db->query("SELECT * FROM form_layout WHERE form_id='$id'");
        $result = $qry->result(); 
        $res = array();
        foreach ($result as $key => $value) {
            $form_id = $value->form_id;
            $form_fields_id = $value->form_fields_id;

            // echo "SELECT * FROM form_fields WHERE id='$form_fields_id'";
            $qry_label = $ci->db->query("SELECT * FROM form_fields WHERE id='$form_fields_id'");
            $label_result = $qry_label->row();  
            // $label_text = $label_result->label_text;
            $field_id = $label_result->field_id;
            $field_name = $label_result->field_name;
            $field_name_english = $label_result->field_name_english;
            $dictionary_link = $label_result->dictionary_link;
            $field_type = $label_result->field_type;
            $validation_class = $label_result->validation_class;
            $input_rules = $label_result->input_rules;
            $input_length = $label_result->input_length;
            $is_required = $label_result->is_required;
            $is_required_cer = $label_result->is_required_cer;
            $place_holder = $label_result->place_holder;
            $placeholder_certificate = $label_result->placeholder_certificate;
            $label_text = $label_result->label_text;
            $is_display = $label_result->is_display;
            $minimum_length = $label_result->minimum_length;
            $id = $label_result->id;
            foreach($label_result as $data)
            {
                $res[$field_id]['field_id'] = $label_result->field_id;
                $res[$field_id]['field_name'] = $label_result->field_name;
                $res[$field_id]['field_name_english'] = $label_result->field_name_english;
                $res[$field_id]['dictionary_link'] = $label_result->dictionary_link;
                $res[$field_id]['validation_class'] = $label_result->validation_class;
                $res[$field_id]['input_rules'] = $label_result->input_rules;
                $res[$field_id]['input_length'] = $label_result->input_length;
                $res[$field_id]['is_required'] = $label_result->is_required;
                $res[$field_id]['is_required_cer'] = $label_result->is_required_cer;
                if($label_result->is_required=='1')
                {
                    $res[$field_id]['is_required']='required';
                }
                else
                {
                    $res[$field_id]['is_required']='';
                }
                if($label_result->is_required_cer=='1')
                {
                    $res[$field_id]['is_required_cer']='required';
                }
                else
                {
                    $res[$field_id]['is_required_cer']='';
                }
                $res[$field_id]['place_holder'] = $label_result->place_holder;
                $res[$field_id]['placeholder_certificate'] = $label_result->placeholder_certificate;
                $res[$field_id]['is_display'] = $label_result->is_display;
                $res[$field_id]['label_text'] = $label_result->label_text;
                $res[$field_id]['minimum_length'] = $label_result->minimum_length;
            }
...
if (!function_exists('get_memtype')) {

    function get_memtype() {
        $ci = & get_instance();
        $ci->db->distinct();
        $ci->db->select('MemberType');
        $ci->db->from('individual_master');
        $ci->db->where('MemberType<>','');
        $ci->db->order_by('MemberType' , 'ASC');
        $result = $ci->db->get()->result_array();
        // echo $ci->db->last_query();
        // $sql = "SELECT distinct member_type FROM `individual` order by member_type ASC";
            // $result = $ci->db->query($sql)->result_array();
...
if (!function_exists('logged_in_user_id')) {
    ...
    function logged_in_user_id() {
        $logged_in_id = 0;
        $CI = & get_instance();
        if ($CI->session->userdata('id') && $CI->session->userdata('username')):
            $logged_in_id = $CI->session->userdata('username');
        endif;
        return $logged_in_id;
    }
